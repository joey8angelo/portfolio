import getNoiseData from "./getNoiseData";
import {
...
export function transform(state: State) {
  const vao = state.active ? state.particlesVao2 : state.particlesVao1;
  const buffer = state.active ? state.particlesBuf1 : state.particlesBuf2;
...
export function render(state: State) {
  const vao = state.active ? state.particlesVao1 : state.particlesVao2;
...
function buildBuffers(state: State) {
  const particles = new Float32Array(state.settings.particleCount * 4);
