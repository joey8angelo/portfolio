 ...
 * https://gist.github.com/toja/0e798c8dd54eb53ea0f0aeef414d16c9
 **/
...
function grad1D(hash: number, x: number) {
...
function grad4D(hash: number, x: number, y: number, z: number, t: number) {
  switch (hash & 31) {
    case 0:
      return x + y;
    case 1:
...
export const perlin1D = function (x: number) {
...
export const perlin2D = function (x: number, y: number) {
...
export const perlin3D = function (x: number, y: number, z: number) {
  ...
  return lerp(
    ...
    lerp(
      v,
      lerp(u, grad3D(aaa, x, y, z), grad3D(baa, x - 1, y, z)),
      lerp(u, grad3D(aba, x, y - 1, z), grad3D(bba, x - 1, y - 1, z)),
    ),
    lerp(
      v,
      lerp(u, grad3D(aab, x, y, z - 1), grad3D(bab, x - 1, y, z - 1)),
      lerp(u, grad3D(abb, x, y - 1, z - 1), grad3D(bbb, x - 1, y - 1, z - 1)),
    ),
  );
};

export const perlin4D = function (x: number, y: number, z: number, t: number) {
  // find unit cube that contains this point
  const X = Math.floor(x) & 255;
  const Y = Math.floor(y) & 255;
  const Z = Math.floor(z) & 255;
  ...
  const aaaa = p[p[p[p[X] + Y] + Z] + T];
  const aaab = p[p[p[p[X] + Y] + Z] + T + 1];
  const aaba = p[p[p[p[X] + Y] + Z + 1] + T];
  const aabb = p[p[p[p[X] + Y] + Z + 1] + T + 1];
  const abaa = p[p[p[p[X] + Y + 1] + Z] + T];
  const abab = p[p[p[p[X] + Y + 1] + Z] + T + 1];
  const abba = p[p[p[p[X] + Y + 1] + Z + 1] + T];
  const abbb = p[p[p[p[X] + Y + 1] + Z + 1] + T + 1];
  const baaa = p[p[p[p[X + 1] + Y] + Z] + T];
  const baab = p[p[p[p[X + 1] + Y] + Z] + T + 1];
const baba = p[p[p[p[X + 1] + Y] + Z + 1] + T];
  const babb = p[p[p[p[X + 1] + Y] + Z + 1] + T + 1];
  const bbaa = p[p[p[p[X + 1] + Y + 1] + Z] + T];
  const bbab = p[p[p[p[X + 1] + Y + 1] + Z] + T + 1];
  const bbba = p[p[p[p[X + 1] + Y + 1] + Z + 1] + T];
  const bbbb = p[p[p[p[X + 1] + Y + 1] + Z + 1] + T + 1];
  ...
  return lerp(
    ...
    lerp(
      ...
      lerp(
        v,
        lerp(u, grad4D(aaaa, x, y, z, t), grad4D(baaa, x - 1, y, z, t)),
        lerp(u, grad4D(abaa, x, y - 1, z, t), grad4D(bbaa, x - 1, y - 1, z, t)),
      ),
      lerp(
        v,
        lerp(u, grad4D(aaba, x, y, z - 1, t), grad4D(baba, x - 1, y, z - 1, t)),
        lerp(
          u,
          grad4D(abba, x, y - 1, z - 1, t),
    ...
    lerp(
      ...
      lerp(
        v,
        lerp(u, grad4D(aaab, x, y, z, t - 1), grad4D(baab, x - 1, y, z, t - 1)),
        lerp(
          u,
          grad4D(abab, x, y - 1, z, t - 1),
      ...
      lerp(
        ...
        lerp(
          u,
          grad4D(aabb, x, y, z - 1, t - 1),
          grad4D(babb, x - 1, y, z - 1, t - 1),
        ),
        lerp(
          u,
          grad4D(abbb, x, y - 1, z - 1, t - 1),
