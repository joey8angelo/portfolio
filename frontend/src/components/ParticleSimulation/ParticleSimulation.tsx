...
export default function ParticleSimulation({
  ...
  const noiseUpdateRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const fpsUpdateRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  // dynamicParticleCountCycles is less than cycles
  // It will run 15 frames cycles times until it finds a suitable particle count
  useEffect(() => {
    if (!dynamicParticleCount || dynamicParticleCountCycles >= cycles) return;
    console.log(
  ...
  // finishes.
  useEffect(() => {
    if (dynamicParticleCountCycles < cycles) return;
    ...
    noiseWorkerRef.current = new Worker(
      new URL("./noise.worker.ts", import.meta.url),
      { type: "module" },
    );
